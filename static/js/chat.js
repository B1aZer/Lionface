...
LionFace.Chat.prototype = {
    ...
    bind_chat : function() {
        ...
        function replaceURLWithHTMLLinks(text) {
            ...
            return text.replace(exp,"<a class='content_link' href='$1' rel='nofollow' target='_blank' title='$1'>$1</a>"); 
        }
        ...
        function save_history(username, tabs) {
            ...
            var tabs = tabs || $('.chat_div');
            var users = [];
            var list_opened = $('#chat_id').data('toggled') || false;
            tabs.each( function (i,e) {
                var user = {};
            ...
            if (users) {
                socket.emit('save history', username, JSON.stringify(users), list_opened); 
            }
        }
        ...
        $(document).on('keypress','#chat_input', function(event) {
            ...
            if (event.keyCode == 13 && $this.val()) {
                ...
                if (usernamecl == 'message_from_'+LionFace.User.username) {
                    ...
                    halfmessage = replaceURLWithHTMLLinks(halfmessage);
